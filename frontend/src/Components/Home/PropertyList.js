...
const PropertyList = () => {
  const [currentPage, setCurrentPage] = useState(1); // Update currentPage state
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      dispatch(propertyAction.updateSearchParams(currentPage)); // Pass currentPage directly
        ...
        <div className="propertylist">
          {properties.map((property) => (
            <Card
              ...
              name={property.propertyName || ""} // Provide a default value if propertyName is undefined
              address={`${property.address.city}, ${property.address.pincode}`}
